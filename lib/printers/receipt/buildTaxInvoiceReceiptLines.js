import {
  ...
  formatReceiptItemTitleLine,
  formatReceiptMoney,
  formatReceiptNoteLine,
 ...
 *
 * @param {{ store: Object, order: Object, invoiceNo: string, documentTitle?: string, includeTender?: boolean }} payload
 */
export function buildTaxInvoiceReceiptLines(payload) {
  const store = payload?.store || {};
  const order = payload?.order || {};
  const invoiceNo = payload?.invoiceNo || "";
  const documentTitle = payload?.documentTitle || "TAX INVOICE";
  const includeTender = payload?.includeTender ?? true;
  const lines = [];
  ...
  items.forEach((item, index) => {
    ...
    pushLine(
      formatReceiptItemTitleLine(sequence, itemName, baseUnitPrice),
    );
    ...
    const noteLine = formatReceiptNoteLine(item.notes);
    if (noteLine) pushLine(noteLine);
  ...
  if (Number(order.surchargeTotal) > 0) {
    pushLine(formatTotalsLine("Surcharges:", order.surchargeTotal), {
      align: "right",
    });
  }

  if (Number(order.processingFee) > 0) {
    pushLine(formatTotalsLine("Surcharge:", order.processingFee), {
      align: "right",
    });
  }
  ...
  if (includeTender && isCashPayment && order.changeDue != null) {
