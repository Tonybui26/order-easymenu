...
export function buildTaxInvoiceReceiptLines(payload) {
  ...
  if (store.storeName) {
    pushLine(store.storeName, { align: "center" });
  }
  pushLine("");
  if (store.phone) {
  ...
  pushLine("TAX INVOICE", { align: "center", bold: true });
  pushLine(RECEIPT_SEP, { align: "center" });
  pushLine(getReceiptOrderTypeLabel(order), { align: "center" });
  pushLine(RECEIPT_SEP, { align: "center" });
  ...
  items.forEach((item, index) => {
    ...
    pushLine(
      `${sequence}. ${itemName} @ ${formatReceiptMoney(unitPrice)}`,
    );
    pushLine(formatQtyLine(qty, unitPrice, lineTotal), { align: "right" });
    ...
    options.forEach((option) => {
      ...
      pushLine(`>${optionName} @ ${formatReceiptMoney(optionPrice)}`);
    });
  });
  ...
  pushLine("Powered by EASYMENU", { align: "center" });
  pushLine("");
