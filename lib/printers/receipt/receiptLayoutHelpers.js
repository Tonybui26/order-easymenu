...
/** Item title row — omit `@ $0.00` when base unit price is zero. */
export function formatReceiptItemTitleLine(sequence, displayName, baseUnitPrice) {
  const prefix = `${sequence}. ${displayName}`;
  const price = Number(baseUnitPrice || 0);
  if (price > 0) {
    return `${prefix} @ ${formatReceiptMoney(price)}`;
  }
  return prefix;
}

/** Modifier/variant line — indented; omit `@ $0.00` when the option has no extra charge. */
export function formatReceiptOptionLine(optionName, optionPrice) {
  ...
  const prefix = `${RECEIPT_MODIFIER_INDENT}>${label}`;
  if (price > 0) {
    return `${prefix} @ ${formatReceiptMoney(price)}`;
  }
  return prefix;
...
/** Item note line — indented under the item title. */
export function formatReceiptNoteLine(note) {
  const text =
    typeof note === "string" ? formatReceiptDisplayName(note.trim()) : "";
  if (!text) return "";
  return `${RECEIPT_MODIFIER_INDENT}Note: ${text}`;
}
