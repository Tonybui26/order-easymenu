...
/** Slightly taller line spacing inside the band (ESC 3 n). Reset with ESC 2 after. */
const ESC_LINE_SPACING_BAND = [0x1b, 0x33, 0x28];
const ESC_LINE_SPACING_DEFAULT = [0x1b, 0x32];
...
function appendInvertedTakeAwayLabel({ addBytes, addText, reverseOn, reverseOff }) {
  ...
  addBytes(reverseOn);
  addBytes(ESC_LINE_SPACING_BAND);
  addText(`${padLine}\n`);
  addText(`${TAKE_AWAY_BAND_LABEL}\n`);
  addText(`${padLine}\n`);
  addBytes(ESC_LINE_SPACING_DEFAULT);
  addBytes(reverseOff);
}
