...
/** Pad row feed before 2×2 label. ESC 2 restores default. */
const ESC_PAD_LINE_SPACING = [0x1b, 0x33, 0x06];
const ESC_LABEL_LINE_SPACING = [0x1b, 0x33, 0x30];
const ESC_DEFAULT_LINE_SPACING = [0x1b, 0x32];

/** Font A 2× width (24 dots/char) label vs Font B normal pad (9 dots/char). */
function getTakeAwayPadLine(labelLine) {
  return " ".repeat(Math.ceil((labelLine.length * 24) / 9));
}
...
function appendInvertedTakeAwayLabel({
  ...
  const padLine = getTakeAwayPadLine(labelLine);

  addBytes(reverseOn);

  if (setPadSize && setLabelSize) {
    addBytes(ESC_PAD_LINE_SPACING);

    setPadSize();
    addText(`${padLine}\n`);

    addBytes(ESC_LABEL_LINE_SPACING);
    setLabelSize();
    addText(`${labelLine}\n`);
