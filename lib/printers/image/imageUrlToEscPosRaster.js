...
/** Target logo width on Print Test (dots). Image is scaled to this width (up or down);
 *  height keeps aspect ratio. Tips: 160 = small, 256 = medium, 384 ≈ 58mm, 512–576 ≈ 80mm. */
...
export async function imageUrlToEscPosRaster(url, options = {}) {
  ...
  try {
    ...
    // Scale to target print width (upscale or downscale). Previously Math.min(max, srcW)
    // capped at the file's pixel width, so raising the constant had no effect on small logos.
    let width = Math.max(8, Math.floor(maxWidthDots / 8) * 8);
