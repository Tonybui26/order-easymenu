 ...
 *
 * Read order when gate on: memory → SQLite → network (then save both).
 * Pass force:true after catalog sync or printer CRUD so the list is not stale.
 * When gate off, always hits the server (unchanged behaviour).
 *
...
export async function checkPrinterAvailability(orderType) {
  ...
  try {
    const printersData = await getPrintersWithLocalCache({ force: false });
