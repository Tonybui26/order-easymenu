...
export function planPrintJobsWithRouting(order, printers, itemGroups) {
  ...
  if (!routingActive) {
    for (const printer of printers) {
      ...
      plan.push({ printer, items: orderItems });
    }
  ...
  for (const printer of printers) {
    ...
    const items =
      groupIds.length === 0
        ? orderItems
        : filterItemsForPrinter(orderItems, printer, itemToGroups);

    if (items.length === 0) continue;
    plan.push({ printer, items });
  }

  const leftoverItems = getLeftoverItems(orderItems, itemToGroups, printers);
