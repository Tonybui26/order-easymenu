...
/**
 * True when a group label should omit the group header on kitchen dockets.
 * - Menu config `skipKitchenDocketGroupHeaders`: skip all group headers.
 * - Per-group `((__))`: skip that group only (unchanged).
 * Customer/staff display still shows the normal group name (e.g. "Size").
 * @param {string|null|undefined} name
 * @param {{ skipAllGroupHeaders?: boolean }} [options]
 * @returns {boolean}
 */
export function shouldSkipPrintGroupHeader(
  name,
  { skipAllGroupHeaders = false } = {},
) {
  if (skipAllGroupHeaders) return true;
...
/**
 * POS item / option label.
 * When useKitchenPrintAliases is true: prefer `((alias))` like kitchen dockets, strip `[[group]]`.
 * When false (default): return the original name unchanged.
 * @param {string|null|undefined} name
 * @param {{ useKitchenPrintAliases?: boolean }} [options]
 * @returns {string}
 */
export function formatPosItemDisplayName(
  name,
  { useKitchenPrintAliases = false } = {},
) {
  if (!useKitchenPrintAliases) return normalizeNameInput(name);
  return stripPrintGroupMarker(getPreferredPrintName(name));
}

/**
 * POS variant/modifier group heading.
 * When useKitchenPrintAliases is true: strip `((...))` / `[[...]]` like storefront.
 * When false (default): return the original name unchanged.
 * @param {string|null|undefined} name
 * @param {{ useKitchenPrintAliases?: boolean }} [options]
 * @returns {string}
 */
export function formatPosGroupDisplayName(
  name,
  { useKitchenPrintAliases = false } = {},
) {
  if (!useKitchenPrintAliases) return normalizeNameInput(name);
