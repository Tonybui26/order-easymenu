 ...
 *
 * Markers may appear on the modifier/variant group name or on individual option names.
 * When both are present, the option marker wins.
 */
...
/**
 * @param {OrderSelection} selection
 * @param {Array<{ id?: string, name?: string }>|null|undefined} itemGroups
 * @returns {{ groupId: string, groupName: string, optionName?: string } | null}
 */
function resolveSelectionPrintRoute(selection, itemGroups) {
  const optionParsed = parsePrintGroupMarker(selection?.optionName);
  if (optionParsed) {
    const groupId = resolvePrintGroupId(optionParsed.marker, itemGroups);
    if (groupId) {
      return {
        groupId,
        groupName: stripPrintGroupMarker(selection?.groupName),
        optionName:
          optionParsed.displayName || stripPrintGroupMarker(selection?.optionName),
      };
    }
  }

  const groupParsed = parsePrintGroupMarker(selection?.groupName);
  if (!groupParsed) return null;

  const groupId = resolvePrintGroupId(groupParsed.marker, itemGroups);
  if (!groupId) return null;

  return {
    groupId,
    groupName:
      groupParsed.displayName || stripPrintGroupMarker(selection?.groupName),
...
export function splitSelectionsByPrintGroup(selections, itemGroups) {
  ...
  for (const selection of selections || []) {
    const route = resolveSelectionPrintRoute(selection, itemGroups);
    if (!route) {
      local.push(selection);
      continue;
    }

    if (!routed.has(route.groupId)) routed.set(route.groupId, []);
    routed.get(route.groupId).push({
      ...selection,
      groupName: route.groupName,
      optionName: route.optionName,
    });
  }
