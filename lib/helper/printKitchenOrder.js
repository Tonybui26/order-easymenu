...
import {
  isPrintToastBridgeEnabled,
  showPrintToast,
} from "@/lib/print/printToastBridge";
import {
  mergeKitchenPrintResults,
  splitKitchenPrintOrders,
} from "@/lib/helper/splitKitchenPrintOrders";
...
function resolvePrintToast(showCustomToast) {
  if (typeof showCustomToast === "function") return showCustomToast;
  if (isPrintToastBridgeEnabled()) return showPrintToast;
  return null;
}
 ...
 * No-ops when store setting `kitchenPrintingEnabled` is off.
 */
export async function printKitchenOrder(
  order,
  {
    ...
    _skipKitchenPrintSplit = false,
  } = {},
) {
  if (!isKitchenPrintingEnabled(menuConfig)) {
    return {
      success: true,
      skipped: true,
      successfulPrints: 0,
      failedPrints: 0,
      totalPrinters: 0,
      message: "Kitchen printing is disabled for this store",
    };
  }

  const resolvedPrintToast = resolvePrintToast(showCustomToast);

  if (!_skipKitchenPrintSplit) {
    const slices = splitKitchenPrintOrders(order);
    if (slices.length > 1) {
      let merged = null;
      for (const slice of slices) {
        const sliceResult = await printKitchenOrder(slice, {
          storeProfile,
          itemGroups,
          menuConfig,
          selectedPrinters,
          ...
          showCustomToast,
          silentNoPrinters,
          _skipKitchenPrintSplit: true,
        });
        merged = mergeKitchenPrintResults(merged, sliceResult);
      }
      ...
      if (merged?.success) {
        ...
        if (merged.failedPrints > 0) {
          if (notify && typeof resolvedPrintToast === "function") {
            showPrintErrorToast(
              resolvedPrintToast,
              buildSimpleUserPrintErrorToast(
                order,
                getFailedPrinterLabelFromResult(merged),
              ),
              buildPrintRetryContext(order, merged, selectedPrinters, source),
            );
          }
          reportPrintFailure(order, merged, "final", source);
        } else if (merged.successfulPrints > 0) {
          reportPrintSuccess(order, merged, source);
        }
      } else if (merged) {
        applyPrintFailureFeedback(order, merged, {
          showCustomToast: resolvedPrintToast,
          attempt: "final",
          notify,
          source,
          printersToUse: selectedPrinters,
          allowRetry: true,
        });
      }
  ...
  try {
    ...
    const skipKitchenDocketGroupHeaders = Boolean(
    ...
    if (printersToUse && printersToUse.length > 0) {
      ...
      if (notify) {
        if (
          ...
          routingActive &&
          backupPrintedItems.length > 0 &&
          typeof resolvedPrintToast === "function"
        ) {
          resolvedPrintToast(
            buildBackupPrintMessage(
              backupPrintedItems,
