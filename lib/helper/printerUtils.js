...
export function formatOrderForPrintingWithESC_POS(
  ...
  order.items.forEach((item) => {
    ...
    const itemName = removeVietnameseDiacritics(item.name);
    ...
    if (item.selectedVariants) {
      ...
      item.selectedVariants.forEach((variant) => {
        ...
        // Apply special filtering for menus that require bracket extraction
        let optionName = removeVietnameseDiacritics(variant.optionName);
        if (needsBracketExtraction) {
    ...
    if (item.selectedModifiers) {
      ...
      item.selectedModifiers.forEach((modifier) => {
        ...
        // Apply special filtering for menus that require bracket extraction
        let optionName = removeVietnameseDiacritics(modifier.optionName);
        if (needsBracketExtraction) {
          optionName = extractBracketContent(optionName);
        }
