...
export function formatOrderForPrintingWithESC_POS(order, orderId) {
  ...
  order.items.forEach((item) => {
    // Font B 2x2 Bold for item name
    // ESC ! 0x31 = Font B (bit 0=1) + double width (bit 5=1) + double height (bit 4=1)
    addBytes([0x1b, 0x45, 0x01]); // ESC E 1 - Bold on
    addBytes([0x1b, 0x21, 0x31]); // ESC ! 0x31 - Font B, 2x2
    addBytes([0x1b, 0x61, 0x00]); // ESC a 0 - Left alignment

    // Wrap item name to prevent word splitting
    const itemName = removeVietnameseDiacritics(item.name);
    const wrappedItemName = wrapText(`${item.quantity} X ${itemName}`, 16);
    ...
    // Variants - Font B 2x2 (no bold) with '>' prefix
    if (item.selectedVariants) {
      addBytes([0x1b, 0x21, 0x31]); // ESC ! 0x31 - Font B, 2x2 (no bold)
      item.selectedVariants.forEach((variant) => {
        ...
        const wrappedVariant = wrapText(variantText, 16);
    ...
    // Modifiers - Font B 2x2 (no bold) with '>' prefix
    if (item.selectedModifiers) {
      ...
      item.selectedModifiers.forEach((modifier) => {
        ...
        const wrappedModifier = wrapText(modifierText, 16);
