...
export function formatOrderForPrintingWithESC_POS(order, orderId) {
  ...
  order.items.forEach((item) => {
    ...
    // To make "extra bold", combine BOLD (bit 3) with double width/height and consider using double-strike mode.
    addBytes([0x1b, 0x21, 0x38]); // ESC ! 0x38 - Font A, 2x2, BOLD
    addBytes([0x1b, 0x47, 0x01]); // ESC G 1 - Double-strike (extra bold ON)
    addBytes([0x1b, 0x61, 0x00]); // ESC a 0 - Left alignment
    ...
    // Variants - Font A double width + bold with '>' prefix and grouped by groupName
    if (item.selectedVariants) {
      addBytes([0x1b, 0x21, 0x28]); // ESC ! 0x28 - Font A, double width + bold
      ...
      item.selectedVariants.forEach((variant) => {
        ...
        variantGroups[groupName].push(
          removeVietnameseDiacritics(variant.optionName),
        );
      });
    ...
    // Modifiers - Font A double width + bold with '>' prefix and grouped by groupName
    if (item.selectedModifiers) {
      ...
      item.selectedModifiers.forEach((modifier) => {
        ...
        modifierGroups[groupName].push(
          removeVietnameseDiacritics(modifier.optionName),
        );
      });
