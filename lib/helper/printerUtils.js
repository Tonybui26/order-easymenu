...
import { getPreferredPrintName, hasPrintNameAlias } from "./printNameAlias.js";
...
function resolvePrintOptionName(rawOptionName, needsBracketExtraction) {
  const preferredName = getPreferredPrintName(rawOptionName);
  let optionName = removeVietnameseDiacritics(preferredName);
  if (needsBracketExtraction && !hasPrintNameAlias(rawOptionName)) {
    optionName = extractBracketContent(optionName);
  }
  return optionName;
}
  ...
  order.items.forEach((item) => {
    ...
    if (item.selectedVariants) {
      ...
      item.selectedVariants.forEach((variant) => {
        const groupName = removeVietnameseDiacritics(variant.groupName);
        if (!variantGroups[groupName]) {
          variantGroups[groupName] = [];
        }
        // Apply print alias first, then legacy bracket extraction when needed
        const optionName = resolvePrintOptionName(
          variant.optionName,
          needsBracketExtraction,
        );
        variantGroups[groupName].push(optionName);
      });

      // Print each group
      Object.entries(variantGroups).forEach(([groupName, options]) => {
        // Print group name with '>' prefix
        addText(` >${groupName}:\n`);
    ...
    if (item.selectedModifiers) {
      ...
      item.selectedModifiers.forEach((modifier) => {
        const groupName = removeVietnameseDiacritics(modifier.groupName);
        ...
        modifierGroups[groupName].push(optionName);
      });

      // Print each group
      Object.entries(modifierGroups).forEach(([groupName, options]) => {
        // Print group name with '>' prefix
        addText(` >${groupName}:\n`);
        // Print each option indented
        options.forEach((option) => {
