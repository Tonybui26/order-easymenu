...
function hasDocketTableNumber(order) {
...
/** Bold centred order-type line on kitchen dockets (includes trailing newlines). */
export function getDocketOrderTypeHeader(order) {
  ...
  if (hasDocketTableNumber(order)) {
    return `Take Away - Table ${String(order.table).trim()}\n\n`;
  }
  return "Take Away\n\n";
}
...
export function formatOrderForPrintingWithESC_POS(
  ...
  // Order type header — bold, centred
