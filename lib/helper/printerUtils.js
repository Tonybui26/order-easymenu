...
export function resolvePrintName(rawName, needsBracketExtraction) {
  ...
  let printName = removeVietnameseDiacritics(stripPrintGroupMarker(preferredName));
...
function formatOrderForReceiptPrinter(order, orderId, menuLink, printer) {
...
export function formatOrderForPrintingWithESC_POS(
  ...
  appendDocketOrderTypeHeader({
    ...
    order,
    setPadSize: () => {
      addBytes([0x1b, 0x45, 0x00]); // bold off — bold widens glyphs, not spaces
      addBytes([0x1b, 0x4d, 0x00]); // Font A
      addBytes([0x1b, 0x21, 0x30]); // same 2×2 as label so rows stack flush
    },
    setLabelSize: () => {
