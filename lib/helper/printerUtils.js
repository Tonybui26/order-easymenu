...
// Aggressive plugin reset function
async function forcePluginReset() {
  console.log("🔧 Starting TCP plugin reset...");

  try {
    // Method 1: Try to create multiple dummy connections to force internal cleanup
    const resetPromises = [];

    for (let i = 0; i < 3; i++) {
      const dummyPromise = TcpSocket.connect({
    ...
    // Wait for all dummy connections to fail
