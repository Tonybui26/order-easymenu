...
export function formatOrderForPrintingWithESC_POS(order, orderId) {
  ...
  order.items.forEach((item) => {
    // Bold and double width for item name
    addBytes([0x1b, 0x45, 0x01]); // ESC E 1 - Bold on
    addBytes([0x1b, 0x21, 0x20]); // ESC ! 32 (0x20) - Double width only
    addBytes([0x1b, 0x61, 0x00]); // ESC a 0 - Left alignment

    // Wrap item name to prevent word splitting (16 chars for double-width font)
