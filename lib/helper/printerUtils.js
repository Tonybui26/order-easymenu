...
import { isStarPrntPrinter } from "../constants/printerLanguages.js";
import {
  createStarPrntCommands,
  formatOrderForPrintingWithStarPrnt,
} from "../printers/starprnt/formatOrderForPrintingWithStarPrnt.js";
import {
  getPreferredPrintName,
  hasPrintNameAlias,
  shouldSkipPrintGroupHeader,
} from "./printNameAlias.js";
import { stripPrintGroupMarker } from "./printGroupMarker.js";
import {
  appendDocketOrderTypeHeader,
  getDocketOrderTypeHeader,
} from "../printers/docketOrderTypeHeader.js";
import { appendKitchenDocketItemNote, getItemPrintNote } from "../printers/itemPrintNote.js";

export { appendDocketOrderTypeHeader, getDocketOrderTypeHeader };
...
/**
 * Wraps text to fit within a specified line width, breaking at word boundaries
 * Prevents words from being split across lines
 * @param {string} text - Text to wrap
 * @param {number} maxWidth - Maximum characters per line (default: 32 for thermal printer)
 * @returns {Array<string>} - Array of wrapped lines
 */
export function wrapText(text, maxWidth = 32) {
...
/**
 * Extracts content inside square brackets from text
 * If brackets are found, returns only the content inside them
 * If no brackets found, returns the original text
...
export function resolvePrintName(rawName, needsBracketExtraction) {
  const preferredName = getPreferredPrintName(rawName);
  let printName = removeVietnameseDiacritics(
    stripPrintGroupMarker(preferredName),
  );
  if (needsBracketExtraction && !hasPrintNameAlias(rawName)) {
    printName = extractBracketContent(printName);
  }
  return printName;
}

/**
 * Array of menuLinks that require special bracket extraction for option names
 * Add menuLinks here to enable bracket content extraction for variants/modifiers
 */
const MENU_LINKS_WITH_BRACKET_EXTRACTION = [
  "toki-toki",
  "phothin-springvale",
  // Add more menuLinks here as needed
  // "another-menu-link",
  // "yet-another-menu",
];

export function needsBracketExtractionForMenuLink(menuLink) {
  return MENU_LINKS_WITH_BRACKET_EXTRACTION.includes(menuLink);
}

function formatOrderForReceiptPrinter(order, orderId, menuLink, printer, printOptions = {}) {
...
function createReceiptTestCommands(message, printer) {
  if (isStarPrntPrinter(printer)) {
...
export function formatOrderForPrintingWithESC_POS(
  order,
  orderId,
  menuLink = null,
  printOptions = {},
  ...
  // Log menuLink for debugging
  console.log("Menu Link:", menuLink);

  // Check if special menuLink handling is needed
  const needsBracketExtraction =
    MENU_LINKS_WITH_BRACKET_EXTRACTION.includes(menuLink);
  const skipGroupHeaderOptions = {
    skipAllGroupHeaders: Boolean(printOptions.skipKitchenDocketGroupHeaders),
  };
  ...
  if (order.isTraining) {
    ...
    addText("TRAINING ONLY\n");
    addBytes([0x1b, 0x45, 0x00]);
  }

  // Order type header — bold, centred (pick-up uses inverted "Take Away")
  ...
  appendDocketOrderTypeHeader({
    ...
    setLabelSize: () => {
      ...
      addBytes([0x1b, 0x4d, 0x00]); // Font A
  ...
  if (orderType === "dine-in") {
    // Dine-in: just show the timestamp
    addText(`${new Date(order.createdAt).toLocaleString()}\n`);
    addText("\n------------------------------------------\n\n");
  } else {
    // Pick-up / Delivery: show full customer and order details
    addText(`${new Date(order.createdAt).toLocaleString()}\n`);
  ...
  order.items.forEach((item) => {
    // Font A 2x2 BOLD for item name (using bit 3 in ESC !)
    // ESC ! 0x38 = Font A (bit 0=0) + double width (bit 5=1) + double height (bit 4=1) + BOLD (bit 3=1)
    addBytes([0x1b, 0x21, 0x38]); // ESC ! 0x38 - Font A, 2x2, BOLD
    addBytes([0x1b, 0x61, 0x00]); // ESC a 0 - Left alignment

    // Wrap item name to prevent word splitting
    ...
    // Variants - Font A 2x2 (no bold) with '>' prefix and grouped by groupName
    if (item.selectedVariants) {
      addBytes([0x1b, 0x21, 0x30]); // ESC ! 0x30 - Font A, 2x2 (no bold)

      // Group variants by raw groupName (identity), print resolved labels
      const variantGroups = {};
      ...
      Object.entries(variantGroups).forEach(([rawGroupName, options]) => {
        if (!shouldSkipPrintGroupHeader(rawGroupName, skipGroupHeaderOptions)) {
    ...
    // Modifiers - Font A 2x2 (no bold) with '>' prefix and grouped by groupName
    if (item.selectedModifiers) {
      addBytes([0x1b, 0x21, 0x30]); // ESC ! 0x30 - Font A, 2x2 (no bold)

      // Group modifiers by raw groupName (identity), print resolved labels
    ...
    if (getItemPrintNote(item)) {
      addBytes([0x1b, 0x21, 0x30]); // ESC ! 0x30 - Font A, 2x2 (no bold)
      appendKitchenDocketItemNote({ addText, wrapText, item });
    }

    // Reset to normal size after each item
    addBytes([0x1b, 0x21, 0x00]); // ESC ! 0x00 - Normal size
    addText("\n");

    // Add dash separator between items
    addBytes([0x1b, 0x61, 0x01]); // ESC a 1 - Center alignment
    addText("------------------------------------------\n");
  });
