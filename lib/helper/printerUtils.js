...
export function formatOrderForPrintingWithESC_POS(order, orderId) {
  ...
  order.items.forEach((item) => {
    // Font A 2x2 Bold for item name
    // ESC ! 0x30 = Font A (bit 0=0) + double width (bit 5=1) + double height (bit 4=1)
    addBytes([0x1b, 0x45, 0x01]); // ESC E 1 - Bold on
    addBytes([0x1b, 0x21, 0x30]); // ESC ! 0x30 - Font A, 2x2
    addBytes([0x1b, 0x61, 0x00]); // ESC a 0 - Left alignment

    // Wrap item name to prevent word splitting
    const itemName = removeVietnameseDiacritics(item.name);
    const wrappedItemName = wrapText(`${item.quantity} X ${itemName}`, 23);
    wrappedItemName.forEach((line, index) => {
      addText(`${index === 0 ? " " : "  "}${line}\n`);
    });

    // Turn off bold before printing variants/modifiers
    addBytes([0x1b, 0x45, 0x00]); // ESC E 0 - Bold off

    // Variants - Font A 2x2 (no bold) with '>' prefix
    if (item.selectedVariants) {
      ...
      item.selectedVariants.forEach((variant) => {
        ...
        const wrappedVariant = wrapText(variantText, 23);
    ...
    // Modifiers - Font A 2x2 (no bold) with '>' prefix
    if (item.selectedModifiers) {
      ...
      item.selectedModifiers.forEach((modifier) => {
        ...
        const wrappedModifier = wrapText(modifierText, 23);
