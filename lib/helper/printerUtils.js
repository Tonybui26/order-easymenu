...
export function formatOrderForPrintingWithESC_POS(order, orderId) {
  ...
  order.items.forEach((item) => {
    ...
    // Variants - Font A double width only (no bold) with '>' prefix and indented
    if (item.selectedVariants) {
      addBytes([0x1b, 0x21, 0x28]); // ESC ! 0x28 - Font A, double width + bold (no double height)
      item.selectedVariants.forEach((variant) => {
        const variantText = `>${removeVietnameseDiacritics(variant.groupName)}: ${removeVietnameseDiacritics(variant.optionName)}`;
        const wrappedVariant = wrapText(variantText, 22);
        wrappedVariant.forEach((line, index) => {
          addText(`${index === 0 ? "  " : "  "}${line}\n`);
        });
      });
    }

    // Modifiers - Font A double width only (no bold) with '>' prefix and indented
    if (item.selectedModifiers) {
      addBytes([0x1b, 0x21, 0x28]); // ESC ! 0x28 - Font A, double width + bold (no double height)
      item.selectedModifiers.forEach((modifier) => {
        const modifierText = `>${removeVietnameseDiacritics(modifier.groupName)}: ${removeVietnameseDiacritics(modifier.optionName)}`;
        const wrappedModifier = wrapText(modifierText, 22);
        wrappedModifier.forEach((line, index) => {
          addText(`${index === 0 ? "  " : "  "}${line}\n`);
        });
      });
    }
