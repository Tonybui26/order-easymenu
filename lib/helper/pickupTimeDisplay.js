...
/** 12:00 AM is stored as 0 — sort after other slots on the same calendar day. */
function getSortMinutesFromMidnight(minutesFromMidnight) {
  if (minutesFromMidnight === 0) return 1439;
...
export function formatScheduledExpectedLabel(order, options = {}) {
  ...
  if (target.dateKey === todayKey) {
    ...
    const totalMinutes = Math.ceil(diffMs / 60000);
    if (totalMinutes <= 60) {
      return `${channel} expected in ${totalMinutes} min${totalMinutes === 1 ? "" : "s"}`;
    }
    return `${channel} expected today at ${target.time12h}`;
  }
...
/**
 * Lexicographic sort key for scheduled orders (earliest fulfilment first).
 * @param {{ pickupTime?: string, fulfillmentSchedule?: { dateKey?: string, minutesFromMidnight?: number }, createdAt?: string }} order
 * @param {{ now?: Date }} [options]
 */
export function getFulfillmentScheduleSortKey(order, { now = new Date() } = {}) {
  const target = resolveFulfillmentTarget(order, now);
  if (target) {
    const minutes = getSortMinutesFromMidnight(target.minutesFromMidnight);
    return `${target.dateKey}T${String(minutes).padStart(4, "0")}`;
  }

  const created = order?.createdAt ? new Date(order.createdAt).getTime() : 0;
  return `9999-12-31T${String(created).padStart(13, "0")}`;
}

/**
 * Compare two orders by fulfilment date then time (earliest first).
 * Midnight (12:00 AM) slots sort after other times on the same day.
 */
export function compareOrdersByFulfillment(a, b, { now = new Date() } = {}) {
  const ak = getFulfillmentScheduleSortKey(a, { now });
  const bk = getFulfillmentScheduleSortKey(b, { now });
  if (ak < bk) return -1;
  if (ak > bk) return 1;
  return 0;
}
