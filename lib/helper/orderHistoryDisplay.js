...
export function isCancelledHistoryOrder(order) {
  return String(order?.status || "").trim() === "cancelled";
}

/** Delivered or cancelled orders shown in Order History. */
export function isHistoryOrder(order) {
  return isDeliveredHistoryOrder(order) || isCancelledHistoryOrder(order);
}

export function isCancelledHistoryRow(orders = []) {
  const list = Array.isArray(orders) ? orders : [orders];
  if (!list.length) return false;
  return list.every((order) => isCancelledHistoryOrder(order));
}
...
/**
 * One destructive action per row: refund when any ticket is still paid
 * (including cancelled paid checks that need a manual refund).
 * Delete only for unpaid non-cancelled rows.
 */
export function resolveHistoryRowPrimaryAction(orders = []) {
  ...
  if (isCancelledHistoryRow(orders)) return null;
...
export function formatOrderHistoryPaymentMethod(paymentMethod) {
  ...
  if (method === "counter-card" || method === "credit-card") return "Card";
  if (!method) return "—";
...
/** Badge key for payment column: partial refund vs fully refunded. */
export function resolveHistoryRowRefundBadge(orders = [], refundSummary) {
  const statuses = new Set(
    orders.map((order) => String(order?.paymentStatus || "").trim()),
  );

  if (statuses.has("partially_refunded")) return "partial";
  if (statuses.has("refunded") && statuses.has("paid")) return "partial";
  if (statuses.has("refunded")) return "refunded";

  if (!refundSummary?.hasRefund) return null;

  if (
    refundSummary.grossTotal > 0 &&
    refundSummary.totalRefunded >= refundSummary.grossTotal
  ) {
    return "refunded";
  }
...
export function formatOrderHistoryRefundBadgeLabel(badge) {
  if (badge === "partial") return "Partial refund";
  if (badge === "refunded") return "Refunded";
  return null;
}
...
/** Email receipt for Online + paid rows (complete or cancelled). */
...
/** Order with a recorded refund — same eligibility as Live Order completed cards. */
export function pickHistoryRefundConfirmationOrder(orders = []) {
  const list = Array.isArray(orders) ? orders : [];
  return (
    list.find((order) => {
      const status = String(order?.paymentStatus || "").trim();
      if (status !== "refunded" && status !== "partially_refunded") return false;
      return order?.refund?.amount != null;
    }) || null
  );
}

/** Show “Send refund confirmation” when any ticket on the row was refunded. */
export function historyRowSupportsSendRefundConfirmation(row) {
  return Boolean(pickHistoryRefundConfirmationOrder(row?.orders));
}
...
export function groupOrdersByTaxInvoiceNo(orders) {
  const eligible = (orders || []).filter(isHistoryOrder);
  ...
  for (const order of eligible) {
    ...
    const groupKey = `${String(order.status || "").trim()}:${invoiceNo}`;
    if (!byInvoice.has(groupKey)) byInvoice.set(groupKey, []);
    byInvoice.get(groupKey).push(order);
  }
...
export function buildOrderHistoryRows(orders, timezone) {
  return groupOrdersByTaxInvoiceNo(orders).map((group) => {
    ...
    const isCancelled = isCancelledHistoryRow(group);
    const refundBadge = resolveHistoryRowRefundBadge(group, refundSummary);
