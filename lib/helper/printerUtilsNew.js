 ...
 *
 * This simulates the real scenario where multiple orders trigger printOrderNewQueued
 * at the same time, testing how the queue handles parallel requests
 */
export async function aggressiveTestNewQueuedParallel(realPrinter, cycles = 5) {
  console.log(
  ...
  try {
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      const jobPromise = printOrderNewQueued(printData, {
        delayAfterDisconnect: 300,
        testing: true,
