...
class PrintQueueManager {
  ...
  async addPrintJob(printData, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      this.queue.push(job);
      console.log(
        `📋 QUEUE: Added print job ${job.id} to queue (position: ${this.queue.length})`,
      );
      ...
      resolve({
        ...
        estimatedWaitTime: (this.queue.length - 1) * this.printDelay,
      });
    });
  }

  /**
   * Process the print queue with delays
   */
  async processQueue() {
    ...
    console.log(
      `📋 QUEUE: Starting queue processing (${this.queue.length} jobs)`,
    );
    ...
    while (this.queue.length > 0) {
      ...
      try {
        console.log(
          `📋 QUEUE: Processing job ${job.id} (${this.queue.length} remaining)`,
        );
      ...
      if (this.queue.length > 0) {
        console.log(
          `📋 QUEUE: Waiting ${this.printDelay}ms before next print...`,
        );
  ...
  setPrintDelay(delayMs) {
    ...
    console.log(`📋 QUEUE: Print delay updated to ${delayMs}ms`);
  }
}

// Global print queue manager
