 ...
 */
import { PRINTER_IP, PRINTER_PORT } from "./printerUtils.js";
import { isTsplPrinter } from "@/lib/constants/printerLanguages";
import {
  createReceiptTestCommands,
  formatOrderForReceiptPrinter,
} from "@/lib/printers/formatReceiptForPrinter.js";
import {
  imageUrlToEscPosRaster,
  prependCenteredLogoToEscPosBase64,
  PRINT_TEST_LOGO_MAX_WIDTH_DOTS,
} from "@/lib/printers/image/imageUrlToEscPosRaster.js";
import { getTsplOrderLabelPayloads } from "@/lib/printers/tspl/createTsplOrderLabels";
import { printTsplLabelPayloads } from "@/lib/printers/printTsplLabelPayloads";
import { isUsbPrinter } from "@/lib/printers/transport/isUsbPrinter";
import { isPrinterReady } from "@/lib/printers/transport/isPrinterReady";
...
export async function printTestNew(
  ...
  try {
    ...
    let printData;
    if (message) {
      printData = createReceiptTestCommands(message, printer);
    } else {
      printData = formatOrderForReceiptPrinter(
        sampleOrder,
        sampleOrderId,
        null,
        printer,
    ...
    // Optional store logo centered above the test receipt content
    const logoUrl = options.logoUrl;
    let logoStatus = { attempted: false, success: false };
    if (logoUrl && !options.onlyConnectionTest) {
      logoStatus.attempted = true;
      try {
        const logoRaster = await imageUrlToEscPosRaster(logoUrl, {
          maxWidthDots: PRINT_TEST_LOGO_MAX_WIDTH_DOTS,
        });
        ...
        console.log("🆕 NEW: Prepended store logo to test print", logoStatus);
        ...
        console.warn(
          "🆕 NEW: Could not print store logo, continuing without it:",
          logoStatus.error,
        );
        if (typeof options.onLogoStatus === "function") {
          options.onLogoStatus(logoStatus);
    ...
    if (isUsbPrinter(printer)) {
      console.log("🆕 NEW: Using USB transport for print test");
      const usbResult = await sendRawViaUsb(printer, printData, {
        ...options,
      ...
      return {
        ...
        message: usbResult.success
          ? message
            ? "Custom test message printed successfully via USB!"
            : "Sample order receipt printed successfully via USB!"
          : usbResult.message,
      };
    }
    ...
    // if onlyConnectionTest is false then send data to printer
    if (!options.onlyConnectionTest) {
...
export async function printOrderNew(printData) {
  const {
    ...
    printers,
    menuLink,
    skipKitchenDocketGroupHeaders = false,
  } = printData;
  let successfulPrints = 0;
  const totalPrinters = printers.length;
  const connectionIds = [];
  const printOptions = { skipKitchenDocketGroupHeaders };
  ...
  try {
    ...
    const failedPrinterErrors = []; // Track error messages for each failed printer
    const successfulPrinterNames = [];
    // Print to all configured printers
    for (const printer of printers) {
      if (printer._id && isPrinterReady(printer) && printer.isActive) {
        let connectionId = null;

        try {
          if (isTsplPrinter(printer)) {
            if (isUsbPrinter(printer)) {
              throw new Error("TSPL label printers do not support USB in v1");
            }
            const payloads = getTsplOrderLabelPayloads({
              order,
              items: order.items,
            });

            if (payloads.length === 0) {
              throw new Error("No items to print on label printer");
            }

            console.log(
              `🆕 NEW: Printing ${payloads.length} TSPL label(s) to ${printer.name} (one TCP job per label)`,
            );

            const tsplResult = await printTsplLabelPayloads(printer, payloads, {
              timeoutMs: 3000,
              delayAfterDisconnectMs: 300,
            });

            if (!tsplResult.success) {
              throw new Error(tsplResult.message);
            }

            successfulPrints++;
            successfulPrinterNames.push(printer.name);
            console.log(
              `🆕 NEW: Successfully printed ${tsplResult.printedCount} label(s) to ${printer.name} (${printer._id})`,
            );
            continue;
          }

          const printDocument = formatOrderForReceiptPrinter(
            order,
            orderId,
            menuLink,
            printer,
          ...
          // USB early branch — existing TCP block below stays unchanged for network printers
          if (isUsbPrinter(printer)) {
            const usbResult = await sendRawViaUsb(printer, printDocument, {
              timeoutMs: 3000,
              delayAfterDisconnect: 300,
            });
            if (!usbResult.success) {
              throw new Error(usbResult.message);
            }
            successfulPrints++;
            successfulPrinterNames.push(printer.name);
            console.log(
              `🆕 NEW: Successfully printed to USB printer ${printer.name} (${printer._id})`,
            );
            continue;
          }

          // Connect to receipt printer using custom plugin
          const connectResult = await PrinterTcpSocketNew.connect({
    ...
    if (successfulPrints > 0) {
      return {
        ...
        failedPrints: totalPrinters - successfulPrints,
        totalPrinters: totalPrinters,
        failedPrinterNames: failedPrinterNames.join(", "),
        failedPrinterErrors: failedPrinterErrors, // Include error details
 ...
 */
export async function printOrderQueued(printData, options = {}) {
  try {
  ...
  try {
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      try {
        ...
        // Add print job to queue
        const queueResult = await printOrderQueued(printData, {
          delayAfterDisconnect: 300,
          testing: true,
        });
...
/**
 * REAL-WORLD SIMULATION: Multiple orders coming in simultaneously
 *
 * This simulates the real scenario where multiple orders trigger printOrderQueued
 * at the same time, testing how the queue handles parallel requests
 */
export async function aggressiveTestQueuedParallel(realPrinter, cycles = 5) {
  console.log(
    `📋 PARALLEL: Simulating ${cycles} orders coming in simultaneously...`,
  );
  ...
  try {
    // ✅ STEP 1: Add ALL jobs to queue simultaneously (like real orders)
    const jobPromises = [];
    const queueStartTime = Date.now();

    console.log(
      `📋 PARALLEL: Adding ${cycles} jobs to queue simultaneously...`,
    );

    for (let i = 0; i < cycles; i++) {
      const sampleOrder = {
        table: `Parallel Table ${i + 1}`,
        createdAt: new Date().toISOString(),
        paymentStatus: "Paid",
        pickupTime: "ASAP",
        ...
        items: [
          {
            ...
            name: `Order ${i + 1} - Item ${Math.floor(Math.random() * 5) + 1}`,
            selectedVariants: [],
            selectedModifiers: [],
          },
        ],
      ...
      const printData = {
        order: sampleOrder,
        orderId: `PARALLEL${String(i + 1).padStart(3, "0")}`,
        printers: [realPrinter],
      };

      // ✅ This simulates real behavior - multiple await calls happening simultaneously
      const jobPromise = printOrderQueued(printData, {
        delayAfterDisconnect: 300,
        testing: true,
    ...
    const queueAddTime = Date.now() - queueStartTime;
    console.log(
      `📋 PARALLEL: All ${cycles} jobs added to queue in ${queueAddTime}ms`,
    );

    // ✅ STEP 2: Wait for all jobs to complete (queue processes them one by one)
    console.log(`📋 PARALLEL: Waiting for queue to process all jobs...`);
    ...
    jobResults.forEach((result, index) => {
      ...
      if (result.success) {
        ...
        results.failedTests++;
        console.log(`❌ Job ${index + 1} failed: ${result.message}`);
      }
    });

    results.totalDuration = Date.now() - results.startTime;

    // Calculate queue behavior metrics
    results.queueBehavior.maxQueueSize = Math.min(
      cycles,
      printQueue.maxQueueSize,
    );
    results.queueBehavior.averageQueueWait = results.totalDuration / cycles;
    results.queueBehavior.queueOverflow = cycles > printQueue.maxQueueSize;

    results.summary = `Parallel test completed: ${results.successfulTests}/${results.totalTests} successful in ${results.totalDuration}ms. Queue handled ${cycles} simultaneous orders.`;

    console.log(`📋 PARALLEL: Results:`, results);
    return results;
  } catch (error) {
    console.error("📋 PARALLEL: Error in parallel aggressive test:", error);
    results.summary = `Parallel test failed: ${error.message}`;
    return results;
  }
}
