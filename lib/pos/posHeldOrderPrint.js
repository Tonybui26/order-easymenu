...
export function buildBillReceiptPayloadFromResumeOrders(
  ...
  const base = buildTaxInvoiceReceiptFromPosCheck({
    ...
    discount: buildReceiptDiscountInput(resumeState.checkDiscount),
  });
...
/**
 * Print kitchen dockets when tickets move to preparing — mirrors Live Order Terminal Prepare.
 * Online/QR orders print only when auto-printing is disabled; counter orders always print.
 */
export async function printHeldCheckKitchenOnPrepare(
  orders,
  preparedOrderIds,
  {
    storeProfile,
  ...
  const preparedSet = new Set((preparedOrderIds || []).map(String));
  const sorted = sortOrdersOldestFirst(orders).filter((order) =>
    preparedSet.has(String(order._id)),
  );

  if (sorted.length === 0) return;

  // Always print on Prepare (recovery when device auto-print missed/failed).
  for (const order of sorted) {
    ...
    if (!result?.success) {
      console.warn(
        `[prepare print] Order ${String(order._id).slice(-6)} failed:`,
        result?.message,
      );
    }
  }
