...
function orderItemToCartLine(
  item,
  orderId,
  { isExternalContext = false, source = undefined } = {},
) {
    ...
    for (const item of order.items || []) {
      lines.push(
        orderItemToCartLine(item, order._id, {
          isExternalContext: asExternalContext,
          source,
        }),
      );
    }
...
/**
 * Mixed table resume: paid QR lines as external context; POS lines as normal check lines.
 */
export function buildMixedTableResumeCartLines(orders) {
  ...
  for (const order of sortedOrders) {
    ...
    lines.push(
      ...buildCartLinesFromResumeOrders([order], {
        asExternalContext: !isPos,
      }),
    );
  }
