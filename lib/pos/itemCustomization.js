...
export function itemHasRequiredModifiers(item, globalModifiers = {}) {
  return (item?.modifierGroups || []).some((group) => {
    if (!group.useGlobal) return false;
    const globalGroup = globalModifiers[group.useGlobal];
    const enabled = getEnabledModifierOptions(group, globalGroup);
    if (enabled.length === 0) return false;
    const { minSelection } = modifierLimits(globalGroup, group);
    return minSelection > 0;
  });
}

/** True when POS customize panel has variant or modifier options to show. */
export function itemHasCustomizableOptions(item, globalModifiers = {}) {
  if (!item) return false;
  const hasVariantOptions =
    item.hasVariants &&
    (item.variants || []).some((group) => (group.options || []).length > 0);
  if (hasVariantOptions) return true;
...
/** True when POS should open the customize panel on add (variants or required modifiers). */
export function itemNeedsCustomization(item, globalModifiers = {}) {
  if (!item) return false;
  const hasVariantOptions =
    item.hasVariants &&
    (item.variants || []).some((group) => (group.options || []).length > 0);
  if (hasVariantOptions) return true;
  return itemHasRequiredModifiers(item, globalModifiers);
}
