...
/** Local-first Send/Pay: offline backup or localDatabase testing. */
...
/** Background upload allowed only when not in localDatabase-only testing. */
export function isOfflineAutoSyncEnabled(menuConfig) {
  return (
    isOfflineSendEnabled(menuConfig) && !isStoreLocalDatabase(menuConfig)
  );
}
