...
async function markSendSynced(localId, serverOrderId, taxInvoiceNo = "") {
  const db = await openLocalDb();
  const now = Date.now();
  await db.run(
  ...
  // Keep invoice on the local payload so receipt print / history can read it.
  const invoice = String(taxInvoiceNo || "").trim();
  if (!invoice) return;
  const result = await db.query(
    `SELECT payload FROM local_orders WHERE local_id = ?`,
    [localId],
  );
  const raw = result?.values?.[0] ? rowField(result.values[0], "payload") : null;
  ...
  payload.taxInvoiceNo = invoice;
  payload.serverOrderId = serverOrderId;
  await db.run(
...
function roundOutboxMoney(amount) {
  return Math.round((Number(amount) || 0) * 100) / 100;
}

function isOutboxCashMethod(method) {
  const normalized = String(method || "").trim();
  return normalized === "cash" || normalized === "counter-cash";
}

function isOutboxCardMethod(method) {
  const normalized = String(method || "").trim();
  return normalized === "credit-card" || normalized === "counter-card";
}

/**
 * Sum unsynced tender amounts (amount due) created at/after `since`.
 * @param {number|Date|string|null} since
 * @param {(method: string) => boolean} methodMatch
 * @returns {Promise<number>}
 */
async function sumPendingOutboxSalesSince(since, methodMatch) {
  const sinceMs = since == null ? 0 : new Date(since).getTime();
  if (!Number.isFinite(sinceMs)) return 0;

  const pending = await listPendingOfflinePayments();
  let total = 0;
  for (const payment of pending) {
    if (!methodMatch(payment.method)) continue;
    if (Number(payment.createdAt) < sinceMs) continue;
    const sale = roundOutboxMoney(
      Number(payment.amountTendered || 0) - Number(payment.changeDue || 0),
    );
    if (sale > 0) total += sale;
  }
  return roundOutboxMoney(total);
}

/**
 * Sum unsynced cash tender amounts (amount due) created at/after `since`.
 * Used for secondTest local register expected so outbox sales match the drawer.
 *
 * @param {number|Date|string|null} since
 * @returns {Promise<number>}
 */
export async function sumPendingOutboxCashSalesSince(since) {
  return sumPendingOutboxSalesSince(since, isOutboxCashMethod);
}

/**
 * Sum unsynced card tender amounts created at/after `since`.
 * @param {number|Date|string|null} since
 * @returns {Promise<number>}
 */
export async function sumPendingOutboxCardSalesSince(since) {
  return sumPendingOutboxSalesSince(since, isOutboxCardMethod);
}
 ...
 * `force: true` uploads even when localDatabase / secondTest hold the outbox
 * (used for sync-then-print Tax Invoice in secondTest).
 */
export function flushOfflineSendOutbox(options = {}) {
  const force = options?.force === true;
  if (localDatabaseOnly && !force) return Promise.resolve();
  if (flushPromise) {
    return flushPromise.finally(() => flushOfflineSendOutbox(options));
  }
  flushPromise = runFlush({ force }).finally(() => {
    flushPromise = null;
  });
  return flushPromise;
}
...
/**
 * Second-test receipt path: upload only these local tickets (and a matching
 * pay row that covers only them) so Mongo assigns taxInvoiceNo. Other outbox
 * rows stay unsynced. Bypasses the local-only hold for this upload only.
 *
 * @param {string[]} localIds
 * @param {{ notify?: boolean }} [options]
 * `notify: false` skips cart remap listeners (print-before-complete must keep
 * localIds on the check so Complete Sale can queue the tender).
 * @returns {Promise<{ success: boolean, taxInvoiceNo?: string, serverOrderIds?: string[], error?: string }>}
 */
export async function syncLocalTicketsForInvoice(localIds, options = {}) {
  ...
  if (ids.length === 0) {
    return { success: false, error: "No local tickets to sync" };
  }
  if (!isLocalDbSupported() || !isLocalCatalogCacheEnabled()) {
  ...
  const targeted = pending.filter((row) =>
    idSet.has(String(rowField(row, "id") || "")),
  );
  ...
  for (const row of targeted) {
    ...
    if (!localId || typeof raw !== "string") {
      return { success: false, error: "Invalid local ticket" };
    }
    ...
    if (!result?.success || !serverOrderId) {
      ...
      return {
        success: false,
        error: result?.error || "Could not sync ticket for an invoice number",
      };
    }

    const invoice = String(result.order?.taxInvoiceNo || "").trim();
    if (invoice && !taxInvoiceNo) taxInvoiceNo = invoice;
    await markSendSynced(localId, serverOrderId, invoice);
    if (notify) {
      notifySynced({
        localId,
        serverOrderId,
  ...
  const resolved = await serverIdsForLocalIds(ids);
  if (!resolved) {
    return {
      success: false,
      error: "Could not sync tickets to the server for an invoice number",
    };
  }

  // Only flush pay rows that belong entirely to this check — leave others queued.
  ...
  for (const row of payments) {
    ...
    const payLocalIds = (payload.localIds || []).map(String);
    const payOrderIds = (payload.orderIds || []).map(String);
    // Match by localIds, or by orderIds when print already remapped the cart.
    const matchesLocal =
      payLocalIds.length > 0 && payLocalIds.every((id) => idSet.has(id));
    const matchesOrder =
      payLocalIds.length === 0 &&
      payOrderIds.length > 0 &&
      payOrderIds.every((id) => resolved.includes(id));
    if (!matchesLocal && !matchesOrder) continue;

    const payResolved =
      payLocalIds.length > 0
        ? await serverIdsForLocalIds(payLocalIds)
        : payOrderIds;
    if (!payResolved) continue;

    const result = await syncOfflinePosPaymentAction({
      ...payload,
      localIds: payLocalIds,
      orderIds: [...new Set([...payOrderIds, ...payResolved])],
    });
  ...
  if (!taxInvoiceNo) {
    const records = await listLocalSendRecords();
    for (const id of ids) {
      const record = records.find((row) => row.localId === id);
      const invoice = String(record?.payload?.taxInvoiceNo || "").trim();
      if (invoice) {
        taxInvoiceNo = invoice;
        break;
      }
  ...
  return {
    success: true,
    taxInvoiceNo,
    serverOrderIds: resolved,
  };
}

/** True when every localId already has a Mongo server_order_id. */
export async function localTicketsAlreadySynced(localIds) {
  const ids = [
    ...new Set(
      (localIds || []).map((id) => String(id || "").trim()).filter(Boolean),
    ),
  ...
  const resolved = await serverIdsForLocalIds(ids);
  return Array.isArray(resolved) && resolved.length === ids.length;
}

/** Reverse-lookup local_ids for Mongo order ids (after a quiet print sync). */
export async function lookupLocalIdsByServerOrderIds(serverOrderIds) {
  const ids = [
    ...new Set(
      (serverOrderIds || [])
  ...
  try {
    ...
    const placeholders = ids.map(() => "?").join(", ");
    const result = await db.query(
      `SELECT local_id, server_order_id FROM local_orders
       WHERE server_order_id IN (${placeholders})`,
      ids,
    );
    return (result?.values || [])
      .map((row) => String(rowField(row, "local_id") || ""))
      .filter(Boolean);
  } catch (error) {
    console.error("lookupLocalIdsByServerOrderIds failed:", error);
    return [];
  }
}
...
async function runFlush({ force = false } = {}) {
  ...
  for (const row of pending) {
    ...
    const taxInvoiceNo = String(result.order?.taxInvoiceNo || "").trim();
    await markSendSynced(localId, serverOrderId, taxInvoiceNo);
    notifySynced({
      localId,
      serverOrderId,
      posCheckId: String(result.order?.posCheckId || payload.posCheckId || ""),
  ...
  await flushReadyPayments({ force });
}
...
async function flushReadyPayments({ force = false } = {}) {
