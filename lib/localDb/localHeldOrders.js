...
function paymentMethodFromRecords(records, paymentByLocalId) {
  for (const record of records) {
    ...
    const fromPay = paymentByLocalId?.get(String(record.localId));
    if (fromPay) return fromPay;
  }
  return null;
}

function buildEntryFromRecords(records, allPaid, paymentByLocalId) {
...
export async function buildLocalPendingHeldEntries() {
  ...
  return [...byCheck.values()]
    .map((group) => {
      const allPaid = group.every(
        (record) =>
          paidLocalIds.has(String(record.localId)) ||
          String(record.payload?.paymentStatus || "").trim() === "paid",
      );
      return buildEntryFromRecords(group, allPaid, methodByLocalId);
    })
