...
/** Gap between notification chimes (LiveOrderTerminal + test sound in More menu). */
export const NOTIFICATION_SOUND_REPLAY_INTERVAL_MS = 1300;
...
/** Single chime — same asset as Live Order Terminal, no repeat loop. */
export async function playNotificationSoundOnce(soundId) {
  if (typeof window === "undefined") return;

  const audio = new Audio(getNotificationSoundUrl(soundId));
  audio.volume = 1;
