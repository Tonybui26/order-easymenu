...
export const TYRO_ICLIENT_LOGS_URL = `${TYRO_SIMULATOR_BASE_URL}/logs.html`;
...
function withTyroTerminalCredentials(requestParams, { mid, tid, integrationKey }) {
...
export function buildTyroPurchaseParams({
  ...
  return withTyroTerminalCredentials(requestParams, {
    mid,
    tid,
    integrationKey,
  });
...
/** Refund params must not include `enableSurcharge`. Amount is inclusive of the original surcharge. */
export function buildTyroRefundParams({
  amount,
  mid,
  tid,
  integrationKey,
  ...
  return withTyroTerminalCredentials(
    {
      amount: amountParam,
      integratedReceipt,
    },
...
export function getTyroIClientWithUI() {
  if (!clientPromise) {
    clientPromise = createTyroIClientWithUI().catch((error) => {
      clientPromise = null;
      throw error;
    });
  }
...
/** Tyro validates callbacks with `[object Function]` — do not pass async functions. */

export function dollarsToTyroCents(dollars) {
  const value = Number(dollars);
  if (!Number.isFinite(value) || value < 0) return null;
  return String(Math.round(value * 100));
}

export function isTyroPurchaseApproved(result) {
  return String(result?.result || "").toUpperCase() === "APPROVED";
}

/**
 * Tyro `surchargeAmount` is a dollar string (e.g. "1.00"), not cents.
 * @returns {number} dollars, 0 if missing/invalid
 */
export function parseTyroSurchargeDollars(result) {
  const raw = result?.surchargeAmount;
...
export function getTyroPurchaseStatusMessage(result) {
  const code = String(result?.result || "").trim();
  if (!code) return "Card payment was not approved";
  if (code === "APPROVED") return "Card payment approved";
  if (code === "CANCELLED") return "Card payment cancelled";
  if (code === "DECLINED") return "Card declined";
  if (code === "REVERSED") return "Card payment reversed";
  return `Card payment ${code.toLowerCase()}`;
}

export function isTyroRefundApproved(result) {
  return isTyroPurchaseApproved(result);
}

export function getTyroRefundStatusMessage(result) {
  const code = String(result?.result || "").trim();
  if (!code) return "Card refund was not approved";
  if (code === "APPROVED") return "Card refund approved";
  if (code === "CANCELLED") return "Card refund cancelled";
  if (code === "DECLINED") return "Card refund declined";
  if (code === "REVERSED") return "Card refund reversed";
  return `Card refund ${code.toLowerCase()}`;
}

/**
 * Headless purchase. Callbacks must be plain functions (not async).
 * @returns {Promise<object>} transactionCompleteCallback payload
 */
export function initiateTyroPurchase(iclient, requestParams) {
  return new Promise((resolve, reject) => {
    try {
      iclient.initiatePurchase(requestParams, {
        receiptCallback: function receiptCallback() {},
        transactionCompleteCallback: function transactionCompleteCallback(
          result,
        ) {
          resolve(result || {});
        },
...
/**
 * Headless refund. Callbacks must be plain functions (not async).
 * Do not pass `enableSurcharge`.
 * @returns {Promise<object>} transactionCompleteCallback payload
 */
export function initiateTyroRefund(iclient, requestParams) {
  return new Promise((resolve, reject) => {
    try {
      iclient.initiateRefund(requestParams, {
        receiptCallback: function receiptCallback() {},
        transactionCompleteCallback: function transactionCompleteCallback(
          result,
        ) {
