  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/order-app/update-modifier-option-availability`,
      {
        ...
        body: JSON.stringify({ sourceType, groupKey, optionId, available }),
      },
    );
...
export async function updateMenuItemSoldOutAction(menuItemId, soldOut) {
  const session = await getServerSession(NextAuthOptions);
  try {
    if (!session) {
      throw new Error("Not authenticated");
    ...
    const response = await fetch(
      `${baseUrl}/api/order-app/update-menu-item-sold-out`,
      {
        ...
        body: JSON.stringify({ menuItemId, soldOut }),
      },
    );
