 ...
 * @param {{ operator?: object, varianceReason?: string }} payload
 */
...
/**
 * Live expected cash for an open register (does not finalise).
 * @returns {Promise<{ success: boolean, closingExpected?: number, cashSalesTotal?: number, sessionId?: string, error?: string }>}
 */
export async function fetchPosRegisterClosingPreviewAction() {
  try {
    return await posRegisterFetch("/api/pos/register/session/closing-preview");
  } catch (error) {
    console.error("fetchPosRegisterClosingPreviewAction error:", error);
    return {
      success: false,
      error: error.message || "Failed to preview closing expected",
    };
  }
}

/**
 * secondTest: auto-finalise (actual = expected from synced cash) then close.
 * @param {{ operator?: object }} payload
 */
export async function closePosRegisterSessionSecondTestAction(payload = {}) {
  try {
    return await posRegisterFetch("/api/pos/register/session/close-second-test", {
      method: "POST",
      body: payload,
    });
  } catch (error) {
    console.error("closePosRegisterSessionSecondTestAction error:", error);
    return {
      success: false,
      error: error.message || "Failed to close register",
    };
