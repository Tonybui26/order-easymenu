...
const PAYMENT_METHODS = [
  {
    ...
    Icon: Banknote,
    className:
      "bg-[#42ecaf] text-[#0f583e] hover:bg-[#6fa33a] active:bg-[#628f34]",
  },
  {
    id: "credit-card",
    label: "Credit Card",
    Icon: CreditCard,
    className:
      "bg-[#e1baff] hover:bg-[#8b7db8] text-[#362864] active:bg-[#7a6ca8]",
  },
];
...
export default function PosPaymentDrawer({
  ...
  useEffect(() => {
    if (!isOpen) return;
    // Placeholder shows amount due; typing starts a fresh tendered amount.
    setDigits("");
  }, [isOpen, amountDue]);
  ...
  function appendToken(token) {
    setDigits((prev) => {
      ...
      if (token === "00") {
        if (prev === "" || prev === "0") return prev === "0" ? "0" : "0";
    ...
    <AnimatePresence>
      ...
      {isOpen ? (
        <motion.aside
          ...
          className="fixed inset-y-0 right-0 z-50 flex w-[min(100%,28rem)] flex-col bg-[#ec7439] pb-[env(safe-area-inset-bottom)] pr-[env(safe-area-inset-right)] pt-[env(safe-area-inset-top)] shadow-2xl"
          initial={{ x: "100%" }}
          animate={{ x: 0 }}
          exit={{ x: "100%" }}
        ...
        >
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
            ...
            <div className="mb-4 flex items-center gap-2">
              <div
                className={cn(
                  "flex min-h-[3.25rem] flex-1 items-center rounded-lg bg-white px-4 text-3xl font-bold tabular-nums",
              ...
              {KEYPAD_ROWS.flat().map((key) => {
                if (key === "backspace") {
                  return (
                    <button
                      ...
                      className="flex h-14 items-center justify-center rounded-md bg-white text-neutral-800 shadow-sm transition-transform active:scale-95"
                      aria-label="Delete"
                    >
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => handleKey(key)}
                    className="flex h-14 items-center justify-center rounded-md bg-white text-xl font-semibold text-neutral-900 shadow-sm transition-transform active:scale-95"
                  >
                    {key}
                  </button>
                );
            ...
            <div className="grid grid-cols-2 gap-2">
              {PAYMENT_METHODS.map(({ id, label, Icon, className }) => (
                <button
                  ...
                  className={cn(
                    "flex min-h-[5.5rem] flex-col items-center justify-center gap-1.5 rounded-lg px-3 py-3 text-sm font-bold uppercase tracking-wide text-white shadow-sm transition-colors",
                    className,
                  )}
                >
